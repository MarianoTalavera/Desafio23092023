...
productRouter.put('/:code', async (req, res) => {
    ...
    try {
        ...
        if (product)
            res.status(200).send({respuesta: 'Producto actualizado', mensaje: product})
        else 
            res.status(404).send({respuesta: 'Error', mensaje: 'Producto no encontrado'})
    } catch (error){
        res.status(400).send({respuesta: 'Error al actualizar el producto', mensaje: error})
    }})
...
productRouter.delete('/:id', async (req, res) => {
    ...
    try {
        ...
        if (product)
            res.status(200).send({respuesta: 'Producto eliminado', mensaje: product})
        else 
            res.status(404).send({respuesta: 'Error', mensaje: 'Producto no encontrado'})
    } catch (error){
        res.status(400).send({respuesta: 'Error', mensaje: error})
